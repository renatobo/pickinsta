...
- **Ollama** (`--scorer ollama`): Self-hosted vision scoring with the same 0-60 rubric. Supports retry/backoff, circuit breaker, and configurable request concurrency. Supported model families: `qwen2.5vl` and `gemma4`. Both use a compact JSON prompt + strict schema; other models get a generic JSON prompt.
  - **Gemma 4 workaround**: Two Ollama bugs affect Gemma 4: (1) `think=false` + `format` parameter causes structured output to be silently ignored ([#15260](https://github.com/ollama/ollama/issues/15260)); (2) the `format` schema causes tier-collapse (all criteria score identically) regardless of prompt variant. Fix: `think` key is **omitted entirely**, and the `format` parameter is **never sent** for any Gemma 4 model. All prompts already instruct JSON output so parsing works without schema enforcement. Also uses `temperature=0.3` (vs 0 for other models). Token budget 280 (vs 512 for Qwen, 220 default). Defaults to `claude+system` prompt variant (Claude-rich prompt + system persona). Handles namespaced models (e.g. `user/gemma4-...`).
...
- `PICKINSTA_OLLAMA_PROMPT_VARIANT` — Gemma 4 prompt variant: `default` or `claude+system` (both, **default**). Only affects Gemma 4 models.
